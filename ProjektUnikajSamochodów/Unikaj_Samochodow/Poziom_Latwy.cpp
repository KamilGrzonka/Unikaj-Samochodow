...
void poziom_latwy::generuj_wroga(int ind) {
	wrogX[ind] = 17 + rand() % (33);
}

void poziom_latwy::rysuj_wroga(int ind) {
	if (wrog_flaga[ind] == 1) {
...
void poziom_latwy::usun_wroga(int ind) {
	if (wrog_flaga[ind] == 1) {
		gotoxy(wrogX[ind], wrogY[ind]);   cout << "    ";
		gotoxy(wrogX[ind], wrogY[ind]);   cout << "    ";
		gotoxy(wrogX[ind], wrogY[ind] + 1); cout << "    ";
...
void poziom_latwy::resetuj_wroga(int ind) {
	usun_wroga(ind);
	wrogY[ind] = 1;
	generuj_wroga(ind);
}

void poziom_latwy::rysuj_granice() {
...
void poziom_latwy::aktualizuj_wynik() {
	gotoxy(WIN_WIDTH + 7, 5); cout << "Wynik: " << wynik << endl;
}

void poziom_latwy::latwy() {
	...
	while (1) {
		...
		if (wynik == 69) {
			gotoxy(WIN_WIDTH + 7, 7); cout << "nice" << endl;
		}
		if (wynik == 71) {
